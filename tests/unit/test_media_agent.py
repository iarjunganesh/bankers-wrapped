...
class TestMediaAgentAssetManifest:
    ...
    async def test_generation_json_includes_sha256_per_artifact(self):
        """ADR-009: every content artifact is listed with key, size, and SHA-256."""
        import hashlib
        ...
        artifacts = uploaded_json["u/s/pipeline/generation.json"]["artifacts"]
        # 12 content artifacts: csv, script, analytics, 5 scenes, thumbnail,
        # prompts, narration, video (generation/metadata manifests excluded)
        assert len(artifacts) == 12
        for entry in artifacts:
            assert entry["key"]
            assert entry["bytes"] > 0
            assert len(entry["sha256"]) == 64
        csv_entry = next(a for a in artifacts if a["key"] == "u/s/input/f.csv")
        assert csv_entry["sha256"] == hashlib.sha256(
