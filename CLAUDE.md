...
**Current phase**: **v2.0.0 released** (2026-07-29) — **submission release**. The ≤3-min demo video is shot, cut and published ([youtu.be/eTw1TCcYFk4](https://youtu.be/eTw1TCcYFk4), 2:50, public, captioned), filmed against the custom domain on canonical CSV run `d987fbba` (90.4 s, 8.7% savings rate). Evidence promoted to `assets/csv-run/d987fbba/`; provider-side corroboration added in `assets/gmi-cloud/`; `submission/COSTS.md` reconciled to measured spend (**$0.17/run**, was an estimated $0.27). `submission/DEVPOST_README.md` verified a byte-exact mirror of `README.md` after URL normalization. **All three required deliverables are shipped**: hosted URL, demo video, and the Devpost entry at [devpost.com/software/banker-s-wrapped](https://devpost.com/software/banker-s-wrapped) (submitted 2026-07-30, public). Remaining work is optional only — the Genblaze Feedback Prize issue, and keeping Railway/Vercel/B2 funded through **Aug 11**. v1.9.2 refreshed genblaze (0.3.7/0.3.6/0.3.4, the upstream "v0.6.0" wave) and all other dependencies, migrated frontend hosting to a custom domain (`bankers-wrapped.arjunganesh.dev`, ADR-012, Railway CORS now explicit instead of wildcard), restructured README badges, added a committed voiceover generator (`scripts/generate_demo_voiceover.py`), and rewrote the submission dossier evidence-first (`submission/SUBMISSION.md`, new `submission/DEVPOST_README.md`). v1.9.1 was a frontend typography/branding fix — real Inter now loads via `next/font/google`, and the "Wrapped" wordmark is a font-independent outlined SVG (`frontend/public/wordmark.svg`, glyphs → vector paths) so it matches the brand cards on every browser without a webfont dependency. v1.9.0 was a maintenance/release-hardening pass plus a full documentation sweep (README restructured, `docs/ARCHITECTURE.md` extracted, theme-aware SVG README banners under `assets/`, demo title/closing cards under `assets/demo-cards/`). All five v1.7.0-roadmap workstreams shipped in **v1.8.0**: WS-1 Genblaze LLM routing (ADR-007), WS-2 B2 source-of-truth (ADR-008), WS-3 lifecycle + SHA-256 integrity (ADR-009), WS-4 Plaid sandbox connector (ADR-010 — httpx REST, no plaid-python; Plaid txns → CSV → unchanged pipeline; feature-flagged off without keys), WS-5 hardening (ADR-011, k6 smoke test, specific-advice prompt, Postgres-claim honesty sweep). **v1.8.1** fixed real NIM routing + fallback. Feature-code freeze in effect.
